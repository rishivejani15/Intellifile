...
function convertFiltersToSearchPayload(filters, currentPath) {
  if (!filters) {
...
function getDefaultFilters(currentPath) {
...
function hasSearchableFilters(filters) {
...
function hasActiveFilters(filters, currentPath = null) {
  if (!filters) return false;
  if (filters.fileType && filters.fileType !== 'all') return true;
  if (filters.datePreset && filters.datePreset !== 'all') return true;
...
function getActiveFilterChips(filters, currentPath = null) {
  if (!filters) return [];
  const chips = [];
