...
function registerIpcHandlers() {
  ...
  ipcMain.handle("chat", async (_, query) => {
    return sendToPython({ action: "chat", query });
  });

  ipcMain.handle("ingest-file", async (_, filePath) => {
    return sendToPython({ action: "ingest_file", file_path: filePath });
  });

  ipcMain.handle("clear-faiss", async () => {
    return sendToPython({ action: "clear_faiss" });
  });
  ...
  // --- AI / Chat / Ingest handlers (merged from root main.js) ---
