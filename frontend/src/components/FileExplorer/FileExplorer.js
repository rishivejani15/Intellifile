...
function FileExplorer({ onFileSelect, selectedFiles = {}, drives = [], onChatWithAI }) {
    ...
    try {
      ...
      if (raw) {
        ...
        if (path) {
          ...
          // Mark cache applied
          setCacheLoaded(true);
          // Refresh the directory silently without showing the loading spinner
          // Use suppressLoading to avoid UI flash; keep trackHistory false to avoid duplicate history entry
  ...
  const handleFolderClick = (item) => {
    if (item.type === 'folder' || item.type === 'drive') {
      setShowPreview(false);
      // Load folder content softly (cached) then refresh in background using existing refresh (F5) logic
      loadDirectory(item.path, { soft: true, trackHistory: true });
      handleRefresh();
    } else if (item.type === 'file') {
      openFileWithDefaultApp(item.path);
