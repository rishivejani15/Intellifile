...
function PreviewPanel({ selectedItem, visible, onClose, onClosing }) {
  const [isClosing, setIsClosing] = useState(false);

  const handleClose = () => {
    if (onClosing) onClosing();
    // Trigger slide‑out animation, then notify parent after it finishes
    setIsClosing(true);
    // Duration matches CSS animation (300 ms)
    setTimeout(() => {
      onClose();
      setIsClosing(false);
    }, 300);
  ...
  return (
    <div className={`preview-panel ${isClosing ? 'closing' : ''}`}>
      <div className="preview-header">
        ...
        <button className="preview-close" onClick={handleClose} title="Close preview">×</button>
      </div>
