...
async def upload_document(file: UploadFile = File(...), clear_previous: bool = Form(False)):
    ...
    # 4. Embed
    try:
        embeddings = embedding_model.encode(chunk_texts, convert_to_numpy=True, normalize_embeddings=True)
    except Exception as e:
...
async def ask_question(request: ChatQueryRequest):
    ...
    search_k = 20 if is_summary else k * 2
    ...
    target_k = 10 if is_summary else k
    ...
    try:
        ...
        resp = requests.post(
            ...
            json={
                "messages": messages,
                "temperature": 0.3 if is_summary else 0.1,
