...
if __name__ == "__main__":
    ...
    # Prevent duplicates: delete existing chunks for the same filenames
    for path in files:
        filename = os.path.basename(path)
        curr.execute("DELETE FROM chunks WHERE source_name = ?", (filename,))
    conn.commit()
    ...
    for path in tqdm(files):
        ...
        for idx, chunk_text_data in enumerate(chunks):
            curr.execute(
                ...
                (doc_id, filename, 0, idx, chunk_text_data, datetime.now(timezone.utc).isoformat())
            )
