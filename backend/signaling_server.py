...
Cloud-ready: reads PORT from environment, exposes /health HTTP endpoint,
supports persistent sessions with 5-minute reconnect window, and basic
rate limiting (10 join attempts per IP per minute).

Run:
    python signaling_server.py          # default port 8787
    PORT=9000 python signaling_server.py
...
class SessionInfo:
    ...
    # Track disconnected slots so peers can rejoin within TTL
    # Maps role (is_initiator bool) → expiry timestamp
    reserved_slots: Dict[bool, float] = field(default_factory=dict)
...
class SignalingHub:
    ...
    async def _cleanup_loop(self) -> None:
        while True:
            ...
            for sid, info in self._sessions.items():
                ...
                expired_roles = [
                    role for role, expiry in info.reserved_slots.items()
                    if now >= expiry
                ]
                for role in expired_roles:
                    del info.reserved_slots[role]
                    log.info("Session %s: reservation for role=%s expired", sid, role)
    ...
    async def _handle_join(self, ws: WebSocketServerProtocol, msg: dict) -> None:
        ...
        # Clear any reservation for this role (peer is reconnecting)
        if is_initiator in info.reserved_slots:
            del info.reserved_slots[is_initiator]
            log.info(
                "Session %s: peer reconnected (role=%s), reservation cleared",
                session_id, is_initiator,
            )
        ...
        if len(info.peers) == 2:
            ...
            log.info("Session %s is now full — both peers connected", session_id)
    ...
    async def _on_disconnect(self, ws: WebSocketServerProtocol) -> None:
        ...
        # Figure out the role of the disconnecting peer for TTL reservation
        disconnected_role = None
        if info:
            for peer in info.peers:
                if peer.socket is ws:
                    disconnected_role = peer.is_initiator
                    break
        ...
        has_reservation = (
            info is not None
            and disconnected_role is not None
            and disconnected_role in info.reserved_slots
        )
    ...
    async def _remove_peer(self, ws: WebSocketServerProtocol, session_id: str) -> None:
        ...
        if info is not None:
            ...
            if removed_peer:
                ...
                info.reserved_slots[removed_peer.is_initiator] = (
...
async def health_check(path, request_headers):
    """Handle HTTP requests for /health — required by cloud platforms."""
    if path == "/health":
        return HTTPStatus.OK, [], b"OK\n"
    # Return None to let websockets handle the connection as usual
    return None
...
async def main() -> None:
    ...
    # Read PORT from environment (cloud platforms set this), fallback to 8787
    port = int(os.environ.get("PORT", os.environ.get("SIGNALING_PORT", "8787")))
